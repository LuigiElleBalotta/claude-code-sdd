import { LauncherStateMachine, type LauncherAction } from './launcher-state-machine.js';
import { RealSessionHost } from './real-session-host.js';
import type { SessionHost, AttachHandle } from './session-host.js';
 ...
 */
export async function runLauncher(projectRoot: string, options: RunLauncherOptions = {}): Promise<number> {
  const host = options.host ?? new RealSessionHost(projectRoot);
  const log = options.log ?? ((message: string): void => void process.stderr.write(`[claude-sdd launch] ${message}\n`));
  const claudeArgs = options.claudeArgs ?? [];
  const machine = new LauncherStateMachine();

  let currentAttach: AttachHandle | undefined;
  ...
  async function perform(action: LauncherAction): Promise<void> {
    switch (action.type) {
      case 'startBackground': {
        log('Starting a new background session...');
        try {
          const { id } = await host.startBackground(claudeArgs);
          for (const next of machine.handle({ type: 'started', id })) await perform(next);
        } catch (err) {
          for (const next of machine.handle({ type: 'startFailed', message: String(err) })) await perform(next);
        }
        return;
      }
      case 'attach': {
        ...
        void currentAttach.exited.then(({ code, signal }) => {
          ...
          for (const next of machine.handle({ type: 'childExited', code, signal })) void perform(next);
        });
        return;
      }
      case 'stopCurrent': {
        ...
        try {
          ...
          for (const next of machine.handle({ type: 'stopFailed', message: String(err) })) await perform(next);
        }
        return;
      }
      case 'warn':
  ...
  for (const action of machine.start()) await perform(action);
