...
export function quoteShellArg(arg: string, platform: NodeJS.Platform = process.platform): string {
  if (arg.length === 0) return platform === 'win32' ? '""' : "''";
...
export function buildCommandLine(command: string, args: readonly string[], platform: NodeJS.Platform = process.platform): string {
