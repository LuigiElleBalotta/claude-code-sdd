...
export function describeForSessionStart(result: SyncResult): string {
  ...
  if (pendingBoundary.length > 0) {
    lines.push(
      `${pendingBoundary.length} specification(s) completed and are waiting for a context boundary: ` +
...
export function describeForStop(result: SyncResult): string | undefined {
  ...
  lines.push(
    'A context boundary has been requested and is persisted in .claude/sdd-state.json. ' +
 ...
 */
export function describeForRestoration(restored: readonly RestoredHandoff[]): string | undefined {
